...
from DBC.datasets.cps_extended import CPSImageDataset
...
def test_model_on_folder(
    ...
    model = get_model(model_type=model_type, num_outputs=num_outputs, device=device, num_channels=num_channels, image_size=480) # 480 was the target width used in training
    model.load_state_dict(torch.load(model_path, map_location=device)["model_state_dict"], strict=False)
    model.to(device)
    model.eval()

    # Load normalization parameters
    if model_type == "vit_extended":
    ...
    with open(dummy_csv_path, "w") as f:
        ...
        for img in image_paths:
            f.write(f"_,{img}," + ",".join([""] * 3) + "," + str(width) + "," + str(height) + ",".join([""] * 7) + "\n")
    ...
    with torch.no_grad():
        for i, (image, _,extra_features) in tqdm(enumerate(test_loader), total=len(test_loader)):
            # Manual normalization

            if model_type == "vit_extended":
                image, extra_features = image.to(device), extra_features.to(device)
                norm_height = (extra_features[:, 0] / max_image_height - 0.5) * 2
                norm_width = (extra_features[:, 1] / max_image_width - 0.5) * 2
                additional_features = torch.stack([norm_height, norm_width], dim=1).to(device)

                output = model(image, additional_features=additional_features)
            else:
                output = model(image)
            output_np = output.detach().cpu().numpy()[0]
            pred = interpret_camera_params(output_np, width, height)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--model_type", default="vit_extended")
    parser.add_argument("--model_path", default="./model/best_model_vit_extended_CPS_2025-05-17.pth")
    ...
    parser.add_argument("--ar", "--aspect_ratio", action="store_true", help="Apply aspect ratio resizing to target_width x target_height")
    parser.add_argument("--target_width", type=int, default=480, help="Target width for resizing")
    parser.add_argument("--target_height", type=int, default=288, help="Target height for resizing")
    parser.add_argument("--width", type=int, default=1392, help="original width")
    parser.add_argument("--height", type=int, default=512, help="original height")
